<%-- 
    Document   : javascript_PM_autoCreateTable
...
<html>
				...
				success: function(response) {
					...
					for(i=0;i<=data.result.records.length;i=i+1){
						for(var key in data.result.records[i]){
                                                        totalStr+=key+" : "+data.result.records[i][key]+" ..";
                                                        if(key=="SiteName"){
					...
					//replaceText(newtest, teststring);
                                        ...
                                        document.insertForm.action="javascript_PM_autoCreateTable.jsp";
                                        document.insertForm.submit();
				},
    ...
    <!--<body onload='setTimeout("subtest()",50000)'>-->
    <body>
        <form name="insertForm" method="post" >
            <input name="totalstr" value="" id="totalstr"></input>
        ...
        <%
            ...
            if(site==null){
               ...
               */
               ...
               //如果沒有配置文件，一定要記得手動宣告
               conf.set("hbase.zookeeper.quorum", "Master");
               String tablename="PM2.522";
               String[] siteString={"基隆","汐止","萬里","新店","土城","板橋","新莊","菜寮","林口","淡水","士林","中山",
...
"永和","復興","埔里","馬祖","金門","馬公","關山","麥寮"};
                    //createTable("PM2.5_real",siteString);
               /*
               {
                    try {
                        ...
                        if (admin.tableExists(tablename)) {
                            ...
                            for(int i=0;i<siteString.length;i++){
                                tableDesc.addFamily(new HColumnDescriptor(siteString[i]));
                            }
               ...
               */
               ...
               ArrayList<String> siteVector = new ArrayList<String>();
               ArrayList<String> PMVector = new ArrayList<String>();
               ...
               int flag=0,cknum=0;
               cknum=totalstr.indexOf("xmg",cknum);
               for(int i=0,pointnum=0,finalnum=0;pointnum>=0;i++){
                   pointnum=totalstr.indexOf("..",pointnum);
                   if(pointnum>(cknum-5)){                 //check sitename is exist
                       break;
                   }else{
                       //==================get_family_and_row======================
                       finalnum=totalstr.indexOf(":",finalnum);
                       totalS=String.valueOf(totalstr.subSequence(pointnum+2,finalnum-1));
                      
                       out.println(totalS);
                       //out.println(cknum);
                       //out.println(pointnum);
                       PMVector.add(totalS);
                       pointnum++;
                       finalnum++;
                       total++;
               ...
               //out.println(siteVector.get(i));
               for(int i=0;i<PMVector.size();i++){
                    try {
                        HBaseAdmin admin = new HBaseAdmin(conf);
                        
                            if (admin.tableExists(PMVector.get(i))) {
                                out.println(PMVector.get(i)+"table already exists!");
                            } else {
                                HTableDescriptor tableDesc = new HTableDescriptor(TableName.valueOf(PMVector.get(i)));

                                for(int j=0;j<siteVector.size();j++){
                                    tableDesc.addFamily(new HColumnDescriptor(siteVector.get(j)));
                                }
                                ...
                                admin.createTable(tableDesc);
                                admin.close();
                                out.println("create table " + PMVector.get(i) + " ok.");
                            }
